...
function loopElements(el,level){
	for(var i=0;i<el.childNodes.length;i++){
		...
		if(el.childNodes[i] && el.childNodes[i]["tagName"] && el.childNodes[i].tagName.toLowerCase() == "li"){
			...
			el.childNodes[i].className = "hiddenbullet"//"myMenu"+level
			...
			for(var j=0;j<childs.length;j++){
				...
				if(temp && temp["tagName"]){
					if(temp.tagName.toLowerCase() == "a"){
						...
						//Onclick event
						//temp.href = "#";
                        if (!el.childNodes[i].a) {
						    temp.onclick = showHide;
						//var to hold the a tag
                            el.childNodes[i].a = temp
						    el.childNodes[i].a.style.display='none'
                        }
						
					}else if(temp.tagName.toLowerCase() == "ul"){
						if(el.childNodes[i].a){ //Add arrow
						    el.childNodes[i].a.style.display='inline'
						    //el.childNodes[i].a.innerHTML="";
                            image = document.createElement("img");
                            image.src=expand_closed;
							el.childNodes[i].a.insertBefore(image,el.childNodes[i].a.childNodes[0])
                            el.childNodes[i].a.removeChild(el.childNodes[i].a.lastChild);
						}
            ...
            if (el.childNodes[i].a.style.display=='none') {
                image = document.createElement("img");
                image.src=expand_item;
			    el.childNodes[i].insertBefore(image,el.childNodes[i].firstChild);
            }
		}
	}
}
	...
	for(var i=0;i<el.childNodes.length;i++){
		...
		if(temp && temp["tagName"] && temp.tagName.toLowerCase() == "ul"){
			...
			if(temp.style.display=="none"){
				temp.style.display = ""
				//Change the arrow:
				if(this.childNodes[0] && this.childNodes[0]["tagName"] && this.childNodes[0].tagName.toLowerCase()=="img"){
                    //image = document.createElement("img");
                    this.childNodes[0].src=expand_open;
					//this.childNodes[0].replaceChild(image,this.childNodes[0].childNodes[0])
				}				
			}else{
				temp.style.display = "none"
				//Change the arrow:
				if(this.childNodes[0] && this.childNodes[0]["tagName"] && this.childNodes[0].tagName.toLowerCase()=="img"){
                    //image = document.createElement("img");
                    this.childNodes[0].src=expand_closed;
					//this.childNodes[0].replaceChild(image,this.childNodes[0].childNodes[0])
				}			
			}
		}
